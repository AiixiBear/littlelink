...
const statusNames = { online: 'オンライン', idle: '退席中', dnd: '取り込み中', offline: 'オフライン' };
const activityTypes = [
  'プレイ中:',      // 0: Playing
  '配信中:',        // 1: Streaming
  '再生中:',        // 2: Listening to
  '視聴中:',        // 3: Watching
  'カスタム:',      // 4: Custom Status
  '参戦中:'         // 5: Competing in
];
...
setInterval(() => {
    ...
    element.textContent = `経過時間: ${formatTime(Date.now() - Number(element.dataset.started))}`;
    });
    updateActivityProgress();
}, 1000);
...
fetch('cloudflare_nodes_ja.json').then(res => res.json())
])
